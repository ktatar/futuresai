---
layout: distill
title: Hugh Xuechen Liu
description: Post-doctoral Fellow
img: assets/img/people/prof_pic_hugh_liu.jpg
importance: 1
category: Affiliated Post-doctoral Fellows
date: 2024-01-01
featured: true
---

<div class="fake-img l-body">
{% include figure.liquid loading="eager" path="assets/img/people/prof_pic_hugh_liu.jpg" title="Hugh Xuechen Liu" %}
</div>

## Bio

> "Study hard what interests you the most in the most undisciplined, irreverent and original manner possible."
> — Richard Feynman

Dr. Hugh Xuechen Liu is an interdisciplinary researcher using **emerging technologies** to create **meaningful experiences for people**.

Currently, Hugh's research explores game studies knowledge discovery, completion, and utilization through AI/ML, with the aim of enabling explainable and accessible game design.

Personal Page: <https://hughxuechen.github.io/>

#AI #GameDesign #ComputationalCreativity #LLM #GenerativeAI #HCI #KnowledgeGraph #Neo4j #Godot #Blockchain #AR #ScienceViz #Gamification
